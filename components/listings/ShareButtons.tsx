...
export default function ShareButtons({ listingId, listingTitle }: ShareButtonsProps) {
  ...
  const url = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://zyai.ro'}/anunt/${listingId}`
  const fbUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}&quote=${encodeURIComponent(listingTitle)}`
  const waUrl = `https://wa.me/?text=${encodeURIComponent(`${listingTitle} — ${url}`)}`
